...
sub print_datadir_stats {
	...
	if ($innodb_buffer_short_percent < $opt{'scale-key_buffer_size'}) {
		...
		bad ("InnoDB total space usage (tables & indexes) ".hr_bytes($innodb_data_size));
		bad ("InnoDB innodb_buffer_pool_size is ".hr_bytes($innodb_buffer_pool_size)." - ". $innodb_buffer_short_percent ."% of the engine usage")
	}
